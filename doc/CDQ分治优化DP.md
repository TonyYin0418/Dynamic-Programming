...
$$
f(i)=\max_{p_j\lt p_i}(f(j)+\min\{cnt[i]-cnt[j],rem[pi]-rem[pj]\})+1
$$
其中，$cnt[i]$ 表示 $a$ 长度为 $i$ 的前缀中没被确定的数字个数，$rem$ 表示 $< p_i$ 且未被确定的数字个数。

注意到这里同时有 $pj<pi$ 和 $j<i$ 两个限制，且中间的 min 可以分类讨论为两种情况：
...
这也就相当于第三个限制（三维偏序）。因此我们先按下标分治，然后左右两边分别按 $cnt[i]-rem[pi]$ 排序，以第一种情况为例，two pointer 扫的时候用 bit 维护 $f(j)-rem[pj]$ 即可。
...
$$
dp[i]=dp[j]+s_{i-1}-s_j+(h_i-h_j)^2;j\lt i\\
dp[i]=dp[j]+s_{i-1}-s_j+h_i^2+h_j^2-2*h_i*h_j\\
(dp[j]-s_j+h_j^2)=h_i\times(2h_j)+(dp[i]-s_{i-1}-h_i^2)
$$

发现能斜率优化，然而斜率 $2h_j$ 单调的条件并不是很好搞，因此考虑 CDQ 分治。第一维为下标，计算 $cdq(l,r)$ 时，左边直接算出来 $[l,mid]$ 中的凸包 ，$[mid+1,r]$ 按照 $h$​ 排序然后计算贡献即可。
